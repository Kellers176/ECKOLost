...
public class blockFloatScr : MonoBehaviour {
	...
	void pushBox()
	{
		...
		xDestination = Mathf.FloorToInt(gameObject.transform.position.x) + Mathf.FloorToInt(20 * xDirection);
		yDestination = Mathf.FloorToInt(gameObject.transform.position.y) + Mathf.FloorToInt(20 * yDirection);
		//I could set the destination to anything. This is just some BS big number.
	}
	...
	void checkObjectCollision()
	{
		...
		{
			gameObject.GetComponent<Rigidbody2D> ().velocity = new Vector2 (-xDirection * 1f, -yDirection * 1f);
			xDestination =  Mathf.FloorToInt(gameObject.transform.position.x);
			yDestination =  Mathf.FloorToInt(gameObject.transform.position.y);
		}
	}
	...
	{
		if (col.gameObject.layer == 12 && gameObject.transform.position == col.transform.position && gameObject.GetComponent<Rigidbody2D>().velocity == Vector2.zero) 
		{
			inHole = true;
